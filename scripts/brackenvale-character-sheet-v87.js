...
console.info("Brackenvale Core character sheet runtime: 0.5.4-test.87");
const TEMPLATE_PATH =
...
Hooks.once("init", () => {
  ...
  class BrackenvaleCharacterSheet extends CharacterActorSheet {
    ...
    _onRender(context, options) {
      ...
      // Visible runtime marker: proves the currently-rendering sheet is this build,
      // rather than merely proving that the module script was downloaded.
      const nav = root.querySelector(".brackenvale-page-tabs");
      if (nav && !nav.querySelector(".brackenvale-runtime-badge")) {
        const badge = document.createElement("span");
        badge.className = "brackenvale-runtime-badge";
        badge.textContent = "v87";
        badge.title = "Brackenvale character-sheet runtime 0.5.4-test.87";
        nav.append(badge);
      }
    ...
    _activateCalibrationControls(root) {
      ...
      toggle?.addEventListener("click", (event) => {
        ...
        root.querySelector(".brackenvale-art-sheet")?.classList.toggle("calibration-mode", this._calibrationMode);
