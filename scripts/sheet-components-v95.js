...
export function prepareSheetComponent(component, actor, moduleId, editable = true) {
  ...
  switch (component.component) {
    ...
    case "page2Region":
      return preparePage2Region(component, actor, editable);
    case "featureList":
...
function preparePage2Region(component, actor, editable) {
  const region = String(component.region ?? "").trim().toLowerCase();

  if (region === "features") {
    const prepared = prepareFeatureList(component, actor, editable);
    return {
      ...prepared,
      isFeatureList: false,
      isPage2FeaturesRegion: true,
      region,
  ...
  if (region === "languages") {
    const prepared = prepareLanguageList(component, actor);
    return {
      ...prepared,
      isLanguageList: false,
      isPage2LanguagesRegion: true,
      region,
      style: createPositionStyle(component)
    };
  }

  if (region === "proficiencies") {
    const prepared = prepareProficiencyList(component, actor);
    return {
      ...prepared,
      isProficiencyList: false,
      isPage2ProficienciesRegion: true,
      region,
      style: createPositionStyle(component)
    };
  }

  console.warn(`brackenvale-core | Unknown Page 2 region: ${component.region}`, component);
  return {
    ...component,
    isPage2UnknownRegion: true,
    style: createPositionStyle(component)
  };
}
