...
console.info("Brackenvale Core character sheet runtime: 0.5.4-test.68");
const TEMPLATE_PATH =
  "modules/brackenvale-core-dev100/templates/character-sheet-v68.hbs";
const LAYOUT_ROOT =
      ...
      const layouts = await Promise.all(
        pageNumbers.map(async (pageNumber) => {
          const response = await fetch(
            `${LAYOUT_ROOT}/page${pageNumber}-v68.json`,
            {cache: "no-store"}
          );
          if (!response.ok) {
            throw new Error(
