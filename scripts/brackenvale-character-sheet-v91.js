...
console.info("Brackenvale Core character sheet runtime: 0.5.4-test.90");
const TEMPLATE_PATH =
  "modules/brackenvale-core/templates/character-sheet-v91.hbs";
const LAYOUT_ROOT =
...
Hooks.once("init", () => {
  ...
  class BrackenvaleCharacterSheet extends CharacterActorSheet {
    ...
    _onRender(context, options) {
      ...
      if (!root.querySelector(".brackenvale-runtime-badge-v89")) {
        const tools = root.querySelector(".brackenvale-layout-tools")
          ?? root.querySelector(".brackenvale-page-tabs");
        if (tools) {
          const badge = document.createElement("span");
          badge.className = "brackenvale-runtime-badge-v89";
          badge.textContent = "v89";
          badge.title = "Brackenvale character sheet runtime 0.5.4-test.89";
          tools.append(badge);
        }
      }
    }
    ...
    _renderPage2DirectDOM(root) {
      ...
      const fieldLayer =
        page.querySelector(".brackenvale-page-fields")
        ?? page;

      fieldLayer.append(overlay);

      console.info(`${MODULE_ID} | Page 2 overlay rendered inside calibration field layer`, {
    ...
    _activateCalibrationControls(root) {
      ...
      root.addEventListener("pointerdown", (event) => {
        ...
        const field = event.target.closest(
          ".brackenvale-page-fields .overlay-field[data-component-key], .brackenvale-page2-dom .overlay-field[data-component-key], .page2-dom-panel[data-component-key]"
        );
    ...
    _setCalibrationFieldState(root) {
      ...
      for (const field of root.querySelectorAll(
        ".brackenvale-page-fields .overlay-field, .brackenvale-page2-dom .overlay-field, .page2-dom-panel[data-component-key]"
      )) {
        if (this._calibrationMode) {
